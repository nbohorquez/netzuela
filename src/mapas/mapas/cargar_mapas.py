...
def analizar_placemark(placemark, tipo_de_placemark):
    ...
    terr[MUNICIPIO] = municipio[0].text.title() if len(municipio) > 0 else ''
    terr[PARROQUIA] = parroquia[0].text.title() if len(parroquia) > 0 else ''
    ...
    """
    terro = Territorio(creador=1, nombre=terr[tipo_de_placemark], poblacion=0, 
    ...
    """
    resultado = sesion.query(territorio.dibujable_p).filter_by(
        territorio_id = terro
    ).first()[0]
    """
    return terro.dibujable.dibujable_id
